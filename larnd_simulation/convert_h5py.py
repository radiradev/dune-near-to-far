...
def process_run(run_id):
    ...
    with h5py.File(filename, 'r') as f:
        ...
        for i_ev in range(len(event_ids)):
            ...
            x,y,z,dQ = HitParser.hit_parser_charge(t0, packets_ev, geom_dict, run_config, switch_xz, drift_model=2)
            ...
            np.savez(f'/pscratch/sd/r/rradev/new_data/{run_id}_{event_idx}.npz', coords=coords, dQ=dQ, paired_data=paired_ev)



# load the paired file
paired = paired_info('/pscratch/sd/r/rradev/larndsim/paired_ndfd.root')
unique_run_ids = np.unique(paired['run'])
eventIDs = paired['eventID']

process_map(process_run, unique_run_ids, max_workers=8)
